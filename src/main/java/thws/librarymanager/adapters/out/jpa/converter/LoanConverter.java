...
public class LoanConverter {
    ...
    public List<LoanEntity> toEntityList(List<Loan> loans) {
        ...
        return loans.stream().filter(Objects::nonNull).map(this::toEntity).collect(Collectors.toList());
    }

    public List<Loan> toDomainList(List<LoanEntity> entities) {
        if (entities == null || entities.isEmpty()) return Collections.emptyList();
        return entities.stream().filter(Objects::nonNull).map(this::toDomain).collect(Collectors.toList());
    }
