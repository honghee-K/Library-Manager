...
class JpaLoanRepositoryIT {
    ...
    void cleanDbAndInit() {
        ...
        book =
                bookPort.save(
                        new Book(null, 1111L, "Test Book", "Author", "Genre", library, null)
                );
    }
    ...
    void save_and_findById() {
        ...
        Optional<Loan> found =
                loanRepository.findById(saved.getId());
    ...
    void existsActiveLoanForBook_returnsTrue_whenActiveLoanExists() {
        ...
        boolean exists =
                loanRepository.existsActiveLoanForBook(book.getIsbn());
    ...
    void findAll_shouldApplyPagingCorrectly() {
        for (int i = 0; i < 15; i++) {
            Book b =
                    bookPort.save(
                            new Book(null, 2000L + i, "Book " + i, "A", "G", library, null)
                    );
        ...
        List<Loan> firstPage =
                loanRepository.findAll(
                        null, null, LoanStatus.ACTIVE, false, 0, 10
                );

        List<Loan> secondPage =
                loanRepository.findAll(
                        null, null, LoanStatus.ACTIVE, false, 1, 10
                );
